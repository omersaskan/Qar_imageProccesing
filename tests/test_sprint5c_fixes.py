...
class TestSprint5CFixes(unittest.TestCase):
    ...
    def test_mobile_high_polycount_reduction(self):
        ...
        # Create a mesh with ~190k faces
        # Sphere with high subdivisions
        ...
        self.assertEqual(stats["decimation_status"], "success")
    ...
    def test_texturing_retry_path_update(self, mock_repair, mock_texturer):
        """
        Verify that retry replaces textured_path and final aligned OBJ comes from retry, not original.
        """
        ...
        # Setup dummy session and cleanup_stats
        session = MagicMock()
        session.session_id = "test_retry"
        session.cleanup_mesh_path = "raw.obj"
