...
def validate_texture(status: str) -> str:
    ...
    if status == "complete":
        return "pass"
    if status in {"minor_missing", "degraded"}:
        return "review"
    return "fail"
...
def validate_contamination(stats: Dict[str, Any], thresholds: ValidationThresholds) -> Dict[str, str]:
    ...
    final_faces = iso.get("final_faces", 0)
    initial_faces = max(iso.get("initial_faces", 1), 1)
    ...
    elif plane_share <= thresholds.max_plane_face_share * 1.5 and plane_v_ratio <= thresholds.max_plane_vertex_ratio * 1.5:
...
def validate_texture_integrity(asset_data: Dict[str, Any]) -> Dict[str, str]:
    results: Dict[str, str] = {}
    texture_status = asset_data.get("texture_status", "missing")
    ...
    if texture_path_exists and not has_uv:
        results["texture_uv_integrity"] = "review"
    elif texture_path_exists and has_uv:
        results["texture_uv_integrity"] = "pass"
    else:
        results["texture_uv_integrity"] = "review" if texture_status != "missing" else "pass"
