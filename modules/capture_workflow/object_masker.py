...
class ObjectMasker:
    ...
    - center prior
    - simple contrast/saliency prior
    - GrabCut refinement
    - contour-based post filtering
    """
    ...
    def _contrast_prior(self, frame: np.ndarray) -> np.ndarray:
        """
        Lightweight saliency approximation:
        compare each pixel to the mean border color in Lab space.
        """
    ...
    def generate_mask(self, frame: np.ndarray) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        try:
            ...
            binary = np.where((gc_mask == cv2.GC_BGD) | (gc_mask == cv2.GC_PR_BGD), 0, 255).astype(np.uint8)
        ...
        occupancy = pixel_count / float(h * w)
    ...
    def process_session(self, frames_dir: str, masks_dir: str) -> List[Dict[str, Any]]:
        ...
        for f_path in frame_files:
            ...
            logger.debug(
                ...
                f"occupancy={meta['occupancy']:.2f}, confidence={meta['confidence']:.2f}"
            )
